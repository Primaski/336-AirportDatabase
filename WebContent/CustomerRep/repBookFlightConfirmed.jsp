...
<%@ page
	import="java.io.*,java.util.*,java.sql.*,java.time.format.*, java.time.LocalDateTime"%>
...
<body>
	<%
		...
		*/
		...
		Object user = session.getAttribute("userHelped");
		try {
			if (user == null) {
				//failure on step 1
		...
		String bookedClass = (bci.equals("3")) ? "economy" : ((bci.equals("2")) ? "business" : "firstClass");

		String query = "SELECT " + "* " + "FROM FlightsExpanded " + "WHERE departAir = '" + departAir + "' and "
				+ "arriveAir = '" + arriveAir + "' and " + "airlineCode = '" + airlineCode + "' and "
				...
				if (flightExpanded.getString(bookedClass + "RemainingCapacity") == null) {
					...
					if (remaining < 1) {
						...
						out.println("Unfortunately, " + bookedClass + " on this flight has no more open "
								+ "seats. Try a different class or flight!");
						return;
					}
				}
			} else {
		...
		try {
			...
			query = "INSERT INTO Tickets (departAir, arriveAir, airlineCode, departTime, "
					+ "departDate, flightClass, generatedOn) " + "values ('" + departAir + "', '" + arriveAir
					+ "', '" + airlineCode + "', '" + departTime + "', '" + departDate + "', '" + bookedClass
					+ "', '" + generatedOn + "')";
		...
		try {
			...
			query = "UPDATE Flights " + "SET " + bookedClass + "RemainingCapacity = if(" + bookedClass
					+ "RemainingCapacity > 0, " + bookedClass + "RemainingCapacity - 1, 0) " + "WHERE departAir = '"
					+ departAir + "' and " + "arriveAir = '" + arriveAir + "' and " + "airlineCode = '"
					+ airlineCode + "' and " + "departDate = '" + departDate + "'";
		...
		try {
			...
			query = "INSERT into Buys (TicketID,Username,price,boughtOn) values ('" + ticketID + "', '"
					+ user.toString() + "', '" + amountPaid + "', '" + generatedOn + "')";
		...
		out.println("<a href=\"customerRepIndex.jsp\">Return to Rep Menu</a>");
	%>
